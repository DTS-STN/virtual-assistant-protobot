 ...
 */
 import { Activity, ActivityTypes, CardFactory, ConversationState, MemoryStorage, TestAdapter, TurnContext, UserState } from 'botbuilder';
 ...
 describe('Unblock Bot Initial', () => {
     ...
     it('Displays text and an image describing where to find banking information for direct deposit auto-enrolement', async () => {
         ...
         assert.strictEqual(reply.attachments[0].content.body[0].text, expectedWelcomeMsg);
         reply =  testAdapter.activityBuffer.shift();
         const expectedOasGreetingMsg = i18n.__('unblock_lookup_update_msg');
         assert.strictEqual(reply.attachments[0].content.body[0].text, expectedOasGreetingMsg);
         reply =  testAdapter.activityBuffer.shift();


         const expectedLookUpMsg = i18n.__('unblock_lookup_update_reason');
         assert.strictEqual(reply.attachments[0].content.body[0].text, expectedLookUpMsg);


         reply =  testAdapter.activityBuffer.shift();
          const confirmMsg = i18n.__('unblock_lookup_update_prompt_msg');

       assert.strictEqual(reply.text, confirmMsg+` (1) Yes, I do or (2) No, I don't`);
